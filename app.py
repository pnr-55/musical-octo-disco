...
# ปรับใหม่เป็นแบบชิดซ้ายสุด ไม่มีเยื้องเข้า ไม่ต้องใช้คำสั่ง if ให้พังอีกต่อไป
...
if img_file_buffer is not None: uploaded_image = img_file_buffer.getvalue()
else:
...
if file_upload is not None: uploaded_image = file_upload.getvalue()
...
chart_data = pd.DataFrame([185, 92, 450], index=["สรีระขาโก่ง (Bowlegs)", "สรีระขานิ่ง (Knock Knees)", "สรีระขาปกติ (Normal)"], columns=["จำนวนผู้ป่วยรวม (ราย)"])
...
if st.button("🔄 รีเซ็ตระบบเพื่อประเมินผู้ป่วยรายใหม่"): st.rerun()
