...
# 🚀 ส่วนหัวระบบยุคอัจฉริยะ (จัดวางใหม่อ่านง่าย สบายตา)
st.markdown("<h2 style='text-align: center; color: #00f2fe; margin-bottom: 0;'>🤖 KNEE-AI SYSTEM</h2>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #6c757d; font-family: monospace; font-size: 14px;'>MULTI-AXIS ALIGNMENT INFERENCE ENGINE // VERSION 7.5</p>", unsafe_allow_html=True)
...
# 🏥 ฐานข้อมูลโรงพยาบาลเครือข่าย 77 จังหวัด
hospitals_data = {
    ...
    "ขอนแก่น": ["โรงพยาบาลขอนแก่น", "โรงพยาบาลศรีนครินทร์"], "เชียงใหม่": ["โรงพยาบาลมหาราชนครเชียงใหม่", "โรงพยาบาลนครพิงค์"],
    "ชลบุรี": ["โรงพยาบาลชลบุรี", "โรงพยาบาลสมเด็จพระบรมราชเทวี ณ ศรีราชา"], "นครราชสีมา": ["โรงพยาบาลมหาราชนครราชสีมา"],
    "ภูเก็ต": ["โรงพยาบาลวชิระภูเก็ต"], "สงขลา": ["โรงพยาบาลสงขลานครินทร์", "โรงพยาบาลหาดใหญ่"]
}
# ป้องกันการพังกรณีจังหวัดอื่น: ดึงชื่อจังหวัดมาตรฐานมารองรับ
all_provinces = ["ลพบุรี", "กรุงเทพมหานคร", "ขอนแก่น", "เชียงใหม่", "ชลบุรี", "นครราชสีมา", "ภูเก็ต", "สงขลา"]
...
# ==========================================
    ...
    if st.session_state.user_data is None:
        ...
        if uploaded_file is not None:
            ...
            # แผงควบคุมฟิลเตอร์ภาพแบบกะทัดรัด
            ...
            if st.button("🤖 เริ่มการคำนวณเชิงลึก (Execute Deep Scan)"):
                ...
                # สูตรจำลองค่าความเสถียรทางการแพทย์สูง
...
# ==========================================
    ...
    if st.session_state.user_data is None or st.session_state.analysis_result is None:
        ...
        # เงื่อนไขการจำแนกสถานะขาทั้ง 3 รูปแบบ
        diagnosis_text = ""
        if angle < 135:
        ...
        st.caption("<div style='text-align: center; color: #6c757d; font-size: 11px; margin-top:15px;'>*ระบบนี้เป็นหุ่นจำลองระบบโทรเวชกรรม (Telemedicine Prototype) สำหรับการศึกษาวิจัยเชิงโครงงานเทคโนโลยีของโรงเรียนพระนารายณ์ ไม่สามารถใช้ทดแทนผลการเอกซเรย์จริงโดยแพทย์*</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    st.markdown("แสดงฐานข้อมูลจำนวนเคสคนไข้จำลองที่พบบ่อยเพื่อช่วยโรงพยาบาลในการวางแผนยาและเวชภัณฑ์")
    ...
    st.info("💡 ประโยชน์ทางการแพทย์: สถิตินี้จะช่วยให้กระทรวงสาธารณสุขหรือโรงพยาบาลประจำจังหวัดสามารถจัดสรรทีมนักกายภาพบำบัดเคลื่อนที่ลงพื้นที่อำเภอพัฒนานิคมได้อย่างแม่นยำขึ้นจ้า")
